...
- (nonnull MTFont*) fontWithMathFontName:(MathFontName) name size:(CGFloat) size
{
    switch (name) {
        case MathFontTypeLatinmodern:
            return [self latinModernFontWithSize:size];
            break;
        case MathFontTypeXits:
            return [self xitsFontWithSize:size];
            break;
        case MathFontTypeTexgyretermes:
            return [self termesFontWithSize:size];
            break;
    }
}
